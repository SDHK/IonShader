...
//获取光照信息 IonStruct_Light（无阴影支持版本）
#define IonLight_MainLight()

//获取光照信息 IonStruct_Light（带阴影支持版本）
...
#define IonLight_ShadowCoord(positionOS, positionCS, positionWS)
...
//返回值：裁剪空间位置（float4）
#define IonShadowCaster_PositionCS(positionOS, normalOS)

//计算从光源位置到顶点的向量（用于距离衰减）
//参数：positionOS - 物体空间位置（float4）
//返回值：向量（float3）
#define IonShadowCaster_Vector(positionOS)
