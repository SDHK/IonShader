...
#if DefPart(IonNoise, Tool) 
#define Def_IonNoise_Tool
...
float IonNoise_Perlin(float2 uv)
{
    ...
    float2 ga = IonHash_22(intPos + float2(0.0, 0.0)); //四角Hash向量
    float2 gb = IonHash_22(intPos + float2(1.0, 0.0));
    float2 gc = IonHash_22(intPos + float2(0.0, 1.0));
    float2 gd = IonHash_22(intPos + float2(1.0, 1.0));
...
float IonNoise_Simple(float2 uv, float time = 0)
{
    ...
    float3 n = h * h * h * h * float3(dot(a, IonHash_22(i)), dot(b, IonHash_22(i + o)), dot(c, IonHash_22(i + 1.0))); // 使用 float3 替换 vec3
...
float IonNoise_Voronoi(float2 uv, float time = 0)
{
    ...
    for (int x = -1; x <= 1; x++) //3x3九宫格采样
    {
        for (int y = -1; y <= 1; y++)
        {
            ...
            float2 pos = IonHash_22(intPos + offset); //生成随机特征点
...
float IonNoise_Worley(float2 uv, float time = 0)
{
    ...
    for (int i = -1; i < 2; i++)
        for (int j = -1; j < 2; j++)
        {
            float2 pos = IonHash_22(index + float2(i, j));
            pos = sin(time + 6.2831 * pos) * 0.5 + 0.5; //特征点随时间变化
...
float IonNoise_FBMvalue(float2 uv)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        value += amplitude * IonNoise_Value(uv); //使用最简单的value噪声做分形，其余同理。
        uv *= 2.0;
        amplitude *= .5;
    }
    return value;
...
float IonNoise_Smoke(float2 uv, float time = 0, float uvSpeed=10, float scale =1, float2 dir = float2(0.0, 0.10))
{
    ...
    uv += dir * time * uvSpeed;
    //水面流动效果
    q.x = IonNoise_FBMvalue(uv + time * dir);
    q.y = IonNoise_FBMvalue(uv + float2(1.0, 0.0));

    //云雾细节效果
    float2 r = float2(0.0, 0.0);
    r.x = IonNoise_FBMvalue(uv + q * scale + time * dir ); // + float2(1.7, 9.2)
    r.y = IonNoise_FBMvalue(uv + q * scale + time * dir ); // + float2(8.3, 2.8)
    
    float f = IonNoise_FBMvalue(uv + r);
