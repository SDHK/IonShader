...
#define Def(name) \
defined(Link_##name) && !defined(Def_##name) || !defined(IonShader)
...
defined(Link_##name) && !defined(Def_##name##_##part) || !defined(IonShader)


// 模块链接宏（单行版本）
// 用于判断是否应该链接某个模块，控制按需加载
#define Link(name) \
defined(Link_##name) || !defined(IonShader)
