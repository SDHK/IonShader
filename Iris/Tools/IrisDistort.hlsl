...
//旋涡扭曲
float2 IrisDistort_Vortex(float2 uv, float distortAngle, float scale = 1, float2 center = (0.5, 0.5))
{
    // 计算 UV 坐标相对于中心点的偏移量,中心指向uv的向量
    float2 offset = uv - center;
...
// center: 扭曲的中心点
// radius: 扭曲的半径
float2 IrisDistort_Sphere(float2 uv, float radius, float2 center = (0.5, 0.5))
{
    // 计算 UV 坐标相对于中心点的偏移量
    float2 offset = uv - center;
...
// frequency: 扭曲的频率
// phase: 扭曲的相位
float2 IrisDistort_Wave(float2 uv, float amplitude, float frequency, float phase)
{
